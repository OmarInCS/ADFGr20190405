...
import model.entities.DepartmentsEOImpl;
...
public class DepartmentsVORowImpl extends ViewRowImpl implements DepartmentsVORow {
    ...
    public void applyEmpRaise(BigDecimal amt){
        RowIterator rows = getEmployees();
        while(rows.hasNext()){
            EmployeesVORowImpl row = (EmployeesVORowImpl) rows.next();
            row.setSalary(row.getSalary().add(amt));
        }
    }
}
